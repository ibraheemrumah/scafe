...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header"><?php echo "Reatail Sales Report"; ?></h1>
		</div>
	</div><!--/.row-->
	<div class="row">
		<div class="col-md-12">
			<div class="panel panel-default">
				<div class="panel-body">
					<form action="<?=base_url()?>reports/retail_sales" method="get">
						<div class="row">
							<div class="col-md-4">
...
<?php
	if(count($pcodeArray) > 0) {
		for($p = 0; $p < count($pcodeArray); $p++) {
			...
			$total_sold_qty 	= 0;
			$total_sales1 = 0;
			$soldItemResult 	= $this->db->query("SELECT qty, price FROM order_items WHERE product_category = '9' AND product_code = '$pcode' AND created_datetime >= '$start_dtm' AND created_datetime <= '$end_dtm' AND status = '1'");
			$soldItemData 		= $soldItemResult->result();
			for($s = 0; $s < count($soldItemData); $s++) {
				$soldItem_qty	= $soldItemData[$s]->qty;
				$soldItem_price	= $soldItemData[$s]->price;
				$total_sold_qty += $soldItem_qty;
				$total_amount = $total_sold_qty * $soldItem_price;
				$total_sales += $total_amount;
				
				unset($soldItem_qty);
			}
			$soldItemResult 	= $this->db->query("SELECT qty, price FROM order_items WHERE product_category = 9 AND  created_datetime >= '$start_dtm' AND created_datetime <= '$end_dtm' ");
			$soldItemData 		= $soldItemResult->result();
			for($s = 0; $s < count($soldItemData); $s++) {
				$soldItem_qty1	= $soldItemData[$s]->qty;
				$soldItem_price	= $soldItemData[$s]->price;
				$total_sold_qty1 += $soldItem_qty;
				$total_amount1 = $total_sold_qty1 * $soldItem_price;
				$total_sales1 += $total_amount;
...
?>		
						...
						<?php echo "Total Retail Sales"; ?>: <?php echo $total_sales1; ?>
					</table>
