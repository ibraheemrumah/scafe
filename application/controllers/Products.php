...
class Products extends CI_Controller
{
    ...
    public function updateProduct()
    {
		...
		$new_qty = strip_tags($this->input->post('item_qty'));
        ...
        if (empty($code)) {
            ...
            $this->Constant_model->updateData('products', $upd_data, $id);
			    $getInvDtaResult = $this->db->query("SELECT * FROM inventory WHERE product_code = '$code' AND outlet_id = '2' ");
    ...
    public function insertProduct()
    {
		...
		$item_qty = strip_tags($this->input->post('item_qty'));
        $us_id = $this->session->userdata('user_id');
        $tm = date('Y-m-d H:i:s', time());
